...
describe(`CommonFormDirective`, () => {
  ...
  @Component({
    ...
    `,
    styles: [`
      :not(form).ng-dirty.ng-touched.ng-invalid {
  ...
  class TestComponent {
    ...
    public request = (formValue: FormValue) => {
      if (formValue.email == 'taken@email.address') {
        return rx.NEVER.pipe(
          rxop.timeoutWith(10, rx.throwError(new HttpErrorResponse({
            status: 422,
            error: {
        ...
        return rx.of('success').pipe(rxop.delay(10))
      } else {
        return rx.NEVER.pipe(
          rxop.timeoutWith(10, rx.throwError(new HttpErrorResponse({
            status: 401,
    ...
    onSubmit (response$: rx.Observable<any>) {
      response$.subscribe(
        r => this.response = r,
  ...
  describe(`form submission`, () => {
    ...
    it(`should correctly interpret server's response as validation`, async () => {
      ...
      expect(form().get('email').errors.serverError).toEqual('That email has already been taken')
      expect(console.error).not.toHaveBeenCalled()
      hasFocus($('[name=email]'))
    })
  ...
  }) // end of second inner describe

}) // end of first outer describe
...
describe(`Settings name properties and attributes`, () => {
  ...
  @Component({
    template: `
      <form [formGroup]="form" commonForm>
